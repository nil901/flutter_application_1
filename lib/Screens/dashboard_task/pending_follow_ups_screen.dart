...
Future<void> pendingFollowUpsApi(
  WidgetRef ref, {
  ...
  final dataList = ref.read(getPendingFollowUprovider.notifier);
  final page = ref.read(currentPageProvider.notifier);
  final hasMore = ref.read(hasMoreDataProvider.notifier);
  ...
  try {
    ...
    final response = await ApiService().getRequest(
      "$getPendingFollowUps?$queryParams",
    );
    ...
    if (response != null && response.statusCode == 200) {
      ...
      final leads = data.map((json) => GetPendingFollowsByMember.fromJson(json)).toList();

      if (isRefresh) {
        dataList.state = leads;
      } else {
        dataList.state = [...dataList.state, ...leads];
      }
...
class PendingFollowUpsScreen extends ConsumerStatefulWidget {
  const PendingFollowUpsScreen({super.key});

  @override
  ConsumerState<PendingFollowUpsScreen> createState() =>
      _PendingFollowUpsScreenState();
}

class _PendingFollowUpsScreenState
    extends ConsumerState<PendingFollowUpsScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() => pendingFollowUpsApi(ref, isRefresh: true));
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        ...
        await pendingFollowUpsApi(ref, isRefresh: true);
      },
      child: Scaffold(
        appBar: CustomAppBar(title: "pending_follow_ups".tr),
        body: Column(
          children: [
Column(
            ...
            Expanded(
              child: Column(
                children: [
                  Expanded(
                    child:pendingProvider.length==0 ?Center(child: Text("No Pending Follow Ups")) : ListView.builder(
                      controller: _scrollController,
                      itemCount: pendingProvider.length + (hasMore ? 1 : 0),
                      itemBuilder: (context, index) {
                        if (index < pendingProvider.length) {
                          final data = pendingProvider[index];
